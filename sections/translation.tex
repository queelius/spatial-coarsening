...
To make the ``special case'' claim concrete for at least one method,
we derive RCTD \citep{cable2021robust} in full as a parametric
specialization of the framework; the remaining methods are connected to
the same ledger but not derived line-by-line here. RCTD models the spot-level UMI count $X_{sj}$ for gene
...
observed spot means at any interior MLE. When $\hat\mu$ is taken
fixed (RCTD's operating mode), the relevant rank condition is on
$\mu$ rather than on $P$: per-spot identifiability of $\bm p_s$ is
governed by $\sigma_{\min}(\mu)$, which is the per-spot rank
condition that \cref{sec:validation} characterizes (the
$\sigma_{\min}(P)$ condition of \cref{thm:rank} governs joint
$\mu$ recovery, which RCTD does not perform).

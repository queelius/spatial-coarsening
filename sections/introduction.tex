...
parametric and non-parametric approaches. Identifiability of
cell-type deconvolution has been studied directly: \citet{cai2024inference}
give asymptotic inference for cell-type proportions from bulk
expression under a full-rank signature assumption, and the
nonnegative-matrix-factorization literature characterizes when a
nonnegative factorization is unique \citep{donoho2003nmf,fu2019nmf}.
Our contribution is not a new identifiability criterion for
deconvolution but a re-derivation of the existing full-column-rank
condition as the coarsening-rank condition of a single masked-data
framework, which lets us read off when each method's assumptions hold.
We caution that ``masked'' here denotes a latent cause hidden inside an
observed candidate set (the Heitjan--Rubin coarsening sense), not the
feature-masking or token-masking objectives of self-supervised machine
learning; the two senses are unrelated. Practical questions remain:
how many marker genes are needed, and when can deconvolution fail
silently?

\subsection{The bridge to masked-data inference}

We observe that ST deconvolution is mathematically an instance of the
...
\item \textbf{A bridge from ST deconvolution to masked-data
  inference} (\cref{sec:translation}), with RCTD worked in full as a
  special case of the framework and the other reference-based methods
  (Cell2location, Tangram, CARD) connected to the same C1--C2--C3 ledger
  through their parametric and procedural choices.

\item \textbf{An identifiability statement organized by the
  framework} (\cref{sec:identifiability}): we recover the known
  full-column-rank condition \citep{cai2024inference,fu2019nmf} as the
  coarsening-rank condition, so that cell-type-specific expression is
  identifiable from spot-level data if and only if the
  spot-by-cell-type composition matrix has full column rank. We
  discuss when this condition is met empirically, and when
  single-cell-resolution platforms or marker-gene panels are needed to
  restore it.

...
The corrected theorem makes two distinct predictions: the
\emph{aggregate} identity ($\hat P^\top R = 0$, $R\hat U = 0$) holds
at any interior MLE unconditionally, while the \emph{per-entry}
identity ($R = 0$) holds only in the saturated regime
$\operatorname{rank}(\bar X) \le K$. We verify both, and the
kernel-dimension fact that separates them, in
\texttt{scripts/cell\_total\_kernel.R}.

\paragraph{Kernel dimension.} The operator $\Lambda: R \mapsto (\hat
P^\top R,\, R\hat U)$ has kernel dimension exactly $(S-K)(J-K)$
across every tested shape (e.g. $(S,K,J) = (2,1,2) \to 1$, $(4,2,3)
\to 2$, $(6,2,5) \to 12$, and $0$ when $K = \min(S,J)$). Joint full
column rank of $\hat P$ and $\hat U$ therefore does \emph{not} make
$\Lambda$ injective: the per-entry identity cannot follow from the
score equations alone whenever $K < \min(S, J)$.

\paragraph{Saturated vs non-saturated.} Fitting the unregularized
spot-mean factorization to a controlled truth with $S = 8$, $J =
10$, $K = 3$: when $\operatorname{rank}(\bar X) = 3 \le K$
(saturated), both the aggregate residuals ($|\hat P^\top R| \le
2\times10^{-13}$) and the per-entry residual ($\max|R| < 10^{-12}$)
vanish. When $\operatorname{rank}(\bar X) = 5 > K$ (non-saturated),
the aggregate residuals still vanish ($|\hat P^\top R| \le
2\times10^{-9}$, $|R\hat U| \le 7\times10^{-15}$) but the per-entry
residual persists at $\max|R| = 0.097$. This is precisely
parts~(a) and~(b) of \cref{thm:cell-total-st}: the saturated
oracle test reproduces the truth entrywise because feeding
$P\mu^\top$ (rank $\le K$ by construction) lands in regime~(b),
whereas a general observed-mean matrix does not.

\paragraph{Noisy regime.} At finite $N_s$ with $\mu$ known, the
oracle per-entry residual is Monte-Carlo noise on top of the
saturated-case zero: across $25$ replicates the median absolute
residual decreases as $0.075$ at $N_s = 100$, $0.024$ at $N_s =
1{,}000$, and $0.0075$ at $N_s = 10{,}000$, consistent with
$1/\sqrt{N_s}$ Poisson scaling.

\paragraph{Practical implication.} A deconvolution method that
matches the observed data in aggregate (always achievable at an
interior MLE) need not match it per cell type. The per-entry
residual is not noise but a structural signature of
$\operatorname{rank}(\bar X) > K$, and disagreement between methods
on per-cell-type estimates is invisible in aggregate-fit checks.
Diagnosing per-cell-type bias requires single-cell-resolution data,
marker-gene calibration, or external biological priors.
...
consistent with the $1/\sqrt{N_s \lambda}$ Poisson floor on relative
residuals (per-cell rate $\lambda \sim 1$ for the top-expressed
genes, giving $1/\sqrt{500} \approx 4.5 \times 10^{-2}$).
...
\citep{tenx2020mousebrain}: $2{,}702$ spots, $32{,}285$ genes,
...
\paragraph{Decomposing the NMF residual.} Write $X_{sj} \sim
\mathrm{Poisson}(N_s \lambda_{sj})$ with per-cell rate $\lambda_{sj}$,
so $\bar X_{sj} = X_{sj}/N_s$ has variance $\mathrm{Var}(\bar X_{sj})
= N_s \lambda_{sj}/N_s^2 = \lambda_{sj}/N_s$ and the plug-in estimate
$\lambda_{sj} \approx \bar X_{sj}$ gives per-entry noise variance
$\bar X_{sj}/N_s$. Aggregating across the matrix yields the
Frobenius-relative floor $\bigl(\sum_{s,j} \bar X_{sj}/N_s
\bigr)^{1/2} / \|\bar X\|_F = 0.053$ on the present data. The observed NMF $K =
